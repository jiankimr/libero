...
def eval_libero(args: Args) -> None:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        # === 500Hz PATCH: Monkey-patch env.step() for sub-step data collection ===
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            dt_sim = None  # 500Hz simulation timestep
            dt_ctrl = None  # 20Hz control timestep
            n_substeps = None
            ...
            if args.save_metrics:
                # === 500Hz: Use simulation timestep instead of control timestep ===
                dt_sim = float(env.env.model_timestep)   # 0.002s (500Hz)
                dt_ctrl = float(env.env.control_timestep) # 0.05s  (20Hz)
                n_substeps = int(dt_ctrl / dt_sim)        # 25
                logging.info(f"[500Hz] Simulation dt = {dt_sim:.6f}s ({1.0/dt_sim:.0f} Hz)")
                logging.info(f"[500Hz] Control dt = {dt_ctrl:.6f}s ({1.0/dt_ctrl:.0f} Hz)")
                logging.info(f"[500Hz] Sub-steps per control step: {n_substeps}")
                
                # Initialize DeltaBuffers at 500Hz resolution
                ...
                try:
                    ...
                    if args.save_metrics:
                        ...
                        for sub_idx, (sub_torque, sub_vel, sub_acc) in enumerate(zip(
                            ...
                            env.env._substep_qacc
                        )):
                            # Torque (commanded torque at this sub-step)
                            torque_list.append(sub_torque)
                            recent_torques.push(sub_torque)
                        
                            # Velocity (joint velocity after this sub-step's integration)
                            velocity_list.append(sub_vel)
                            recent_velocities.push(sub_vel)
                        
                            # Acceleration (joint acceleration during this sub-step)
                            acceleration_list.append(sub_acc)
                            recent_joint_acc.push(sub_acc)
                        
                            # Kinematic Jerk (DeltaBuffer delta method) at 500Hz
                            kinematic_jerk_delta = recent_joint_acc.delta / dt_sim
                            kinematic_jerk_delta_list.append(kinematic_jerk_delta)
                            
                            # Actuator Jerk (DeltaBuffer delta method) at 500Hz
                            ...
                            E = P * dt_sim  # Energy [J] at simulation timestep
                            electric_energy_list.append(E)
                        
                        if args.debug_action:
                            logging.debug(
                                f"[500Hz] Collected {len(env.env._substep_torques)} sub-steps, "
                                f"total points so far: {len(torque_list)}"
                            )
            ...
            # Compute and save physical metrics if enabled (500Hz resolution)
            if args.save_metrics:
                try:
                    # Log 500Hz data summary
                    logging.info(
                        f"[500Hz] Episode {episode_idx}: {len(torque_list)} total sub-step data points "
                        f"(≈ {len(torque_list) / n_substeps:.0f} control steps × {n_substeps} sub-steps)"
                    )
                    
                    # Save basic metrics (npy files) at 500Hz resolution
                    compute_basic_metrics(
                        ...
                        dt=dt_sim,  # 500Hz timestep (0.002s)
                    ...
                    # Also save torque_current (= torque at 500Hz, same as torque_list)
                    # for backwards compatibility with life ratio calculation
                    if len(torque_list) > 0:
                        torque_current_array = np.array(torque_list)
                        pathlib.Path(analysis_out_path).mkdir(parents=True, exist_ok=True)
                        np.save(
                            pathlib.Path(analysis_out_path) / f"torque_current_{task_segment}_{episode_idx}_{suffix}.npy",
                            torque_current_array
                        )
                        logging.info(f"Saved torque_current (500Hz): shape {torque_current_array.shape}")
                    logging.info(f"Basic metrics saved for episode {episode_idx} at 500Hz")
                    ...
                    episode_metric_dict = {
                        ...
                        "dt": dt_sim,  # 500Hz simulation timestep
